...
def failure_panels(
    ...
    for sample_no, prediction in enumerate(predictions):
        ...
        for column, target in ((1, "pred"), (2, "true")):
            path = output_dir / "gradcam" / f"{sample_id}_{target}.npy"
            if path.exists():
        ...
        if rows:
            ...
            last = output_dir / "spatial" / sample_id / f"{last_label}.npz"
            if last.exists():
        ...
        sample_scale = [row for row in scale_rows if int(row.get("sample_id", -1)) == sample_no]
        if sample_scale:
        ...
        erf_path = output_dir / "erf" / f"{sample_id}_stem.npy"
        if erf_path.exists():
